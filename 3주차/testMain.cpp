...
int main() {
	...
	uniquePtr seat = loadSeat(row, col, reserved);
	if (seat != nullptr) {
...
void showSeat(uniquePtr(&seat), const int ROW, const int COL) {
	cout << "******* 좌석 현황 *****" << endl;
	cout << '\t';
	for (int i = 1; i <= COL; i++) {
		cout << i << '\t';
...
void reserveSeat(uniquePtr(&seat), const int ROW, const int COL, int& reserved) {
...
void cancelSeat(uniquePtr(&seat), const int row, const int col, int& reserved) {
	showSeat(seat, row, col);
	cout << "좌석 입력하세요" << endl;
	char x, y;
	cin >> x >> y;
...
void saveSeat(const string& filename, uniquePtr(&seat), const int row, const int col) {
...
void menu(const string& filename, uniquePtr(&seat),
...
uniquePtr loadSeat(int& row, int& col, int& reserved) {
	...
	uniquePtr seat = make_unique<unique_ptr<int[]>[]>(row);
	for (int i = 0; i < row; i++)
